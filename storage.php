...
if ( ! function_exists( 'tuning_storage_get' ) ) {
	/**
	 ...
	 */
	function tuning_storage_get( $var_name, $default = '' ) {
		global $TUNING_STORAGE;
		return isset( $TUNING_STORAGE[ $var_name ] ) ? $TUNING_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'tuning_storage_set' ) ) {
	/**
	 ...
	 */
	function tuning_storage_set( $var_name, $value ) {
		global $TUNING_STORAGE;
		$TUNING_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'tuning_storage_empty' ) ) {
	/**
	 ...
	 */
	function tuning_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $TUNING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $TUNING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $TUNING_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $TUNING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'tuning_storage_isset' ) ) {
	/**
	 ...
	 */
	function tuning_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $TUNING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $TUNING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $TUNING_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $TUNING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'tuning_storage_unset' ) ) {
	/**
	 ...
	 */
	function tuning_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $TUNING_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $TUNING_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $TUNING_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $TUNING_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'tuning_storage_inc' ) ) {
	/**
	 ...
	 */
	function tuning_storage_inc( $var_name, $value = 1 ) {
		global $TUNING_STORAGE;
		if ( empty( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = 0;
		}
		$TUNING_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'tuning_storage_concat' ) ) {
	/**
	 ...
	 */
	function tuning_storage_concat( $var_name, $value ) {
		global $TUNING_STORAGE;
		if ( empty( $TUNING_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'tuning_storage_get_array' ) ) {
	/**
	 ...
	 */
	function tuning_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $TUNING_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $TUNING_STORAGE[ $var_name ][ $key ] ) ? $TUNING_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $TUNING_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $TUNING_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'tuning_storage_set_array' ) ) {
	/**
	 ...
	 */
	function tuning_storage_set_array( $var_name, $key, $value ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$TUNING_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'tuning_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function tuning_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $TUNING_STORAGE[ $var_name ][ $key ] ) ) {
			$TUNING_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$TUNING_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$TUNING_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'tuning_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function tuning_storage_merge_array( $var_name, $key, $value ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$TUNING_STORAGE[ $var_name ] = array_merge( $TUNING_STORAGE[ $var_name ], $value );
		} else {
			$TUNING_STORAGE[ $var_name ][ $key ] = array_merge( $TUNING_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'tuning_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function tuning_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			tuning_array_insert_after( $TUNING_STORAGE[ $var_name ], $after, $key );
		} else {
			tuning_array_insert_after( $TUNING_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'tuning_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function tuning_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			tuning_array_insert_before( $TUNING_STORAGE[ $var_name ], $before, $key );
		} else {
			tuning_array_insert_before( $TUNING_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'tuning_storage_push_array' ) ) {
	/**
	 ...
	 */
	function tuning_storage_push_array( $var_name, $key, $value ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $TUNING_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $TUNING_STORAGE[ $var_name ][ $key ] ) ) {
				$TUNING_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $TUNING_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'tuning_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function tuning_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $TUNING_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $TUNING_STORAGE[ $var_name ] ) && is_array( $TUNING_STORAGE[ $var_name ] ) && count( $TUNING_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $TUNING_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $TUNING_STORAGE[ $var_name ][ $key ] ) && is_array( $TUNING_STORAGE[ $var_name ][ $key ] ) && count( $TUNING_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $TUNING_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'tuning_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function tuning_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( empty( $TUNING_STORAGE[ $var_name ][ $key ] ) ) {
			$TUNING_STORAGE[ $var_name ][ $key ] = 0;
		}
		$TUNING_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'tuning_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function tuning_storage_concat_array( $var_name, $key, $value ) {
		global $TUNING_STORAGE;
		if ( ! isset( $TUNING_STORAGE[ $var_name ] ) ) {
			$TUNING_STORAGE[ $var_name ] = array();
		}
		if ( empty( $TUNING_STORAGE[ $var_name ][ $key ] ) ) {
			$TUNING_STORAGE[ $var_name ][ $key ] = '';
		}
		$TUNING_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'tuning_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function tuning_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $TUNING_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $TUNING_STORAGE[ $var_name ] ) ? $TUNING_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $TUNING_STORAGE[ $var_name ] ) ? $TUNING_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'tuning_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function tuning_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $TUNING_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $TUNING_STORAGE[ $var_name ]->$prop ) ? $TUNING_STORAGE[ $var_name ]->$prop : $default;
	}
}
