<div class="front_page_section front_page_section_testimonials<?php
	$tuning_scheme = tuning_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $tuning_scheme ) && ! tuning_is_inherit( $tuning_scheme ) ) {
		echo ' scheme_' . esc_attr( $tuning_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( tuning_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( tuning_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$tuning_css      = '';
		$tuning_bg_image = tuning_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$tuning_anchor_icon = tuning_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$tuning_anchor_text = tuning_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( tuning_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$tuning_css      = '';
			$tuning_bg_mask  = tuning_get_theme_option( 'front_page_testimonials_bg_mask' );
			$tuning_bg_color_type = tuning_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $tuning_bg_color_type ) {
				$tuning_bg_color = tuning_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$tuning_caption = tuning_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $tuning_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $tuning_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $tuning_caption, 'tuning_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$tuning_description = tuning_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $tuning_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $tuning_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $tuning_description ), 'tuning_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! tuning_exists_trx_addons() ) {
						...
						tuning_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
