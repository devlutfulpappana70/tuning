...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			tuning_create_observer( 'tuning-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				tuning_gutenberg_editor_init();
			} );
		} else {
		 	tuning_gutenberg_editor_init();
		}
	...
	function tuning_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'tuning_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'tuning_inited' ) ) {
	...
	// Init on page load
	function tuning_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = tuning_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + TUNING_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + TUNING_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in TUNING_STORAGE['override_classes'] ) {
				$obj.addClass( TUNING_STORAGE['override_classes'][i].replace( '%s', TUNING_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( tuning_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && TUNING_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'tuning_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			tuning_remove_observer( 'tuning-check-editor-styles-wrapper' );
			tuning_create_observer( 'tuning-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'tuning_inited' );
	}
} );
