...
jQuery( document ).ready(
	function() {
		...
		tuning_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="tuning_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.tuning_post_editor' );
						if ( link.length > 0 ) {
							tuning_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.tuning_post_editor', function(e) {
						tuning_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('tuning_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function tuning_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
